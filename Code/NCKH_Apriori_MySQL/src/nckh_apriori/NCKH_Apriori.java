 ...
 * Thuật toán : Apriori
 * Mô tả thuật toán : 
 * Độ phức tạp của thuật toán : O(nm2^m) //n : Sô itemset, m : số item (tham khảo Vipin Kumar)
 */
...
public class NCKH_Apriori {
    ...
    public static void main(String[] args) throws Exception {
        ...
        //<editor-fold defaultstate="collapsed" desc="Đọc các itemset từ MySQL server">

        MySQL inStream = new MySQL(server,dbName,dbUser,dbPassword);
        inStream.connect();
        ResultSet rs = inStream.select("select transactions from transactions"+dbLimit);
        ...
        while (rs.next()) {
            ...
            //stToken.nextToken(); //Enabel when using file input
        ...
        //<editor-fold defaultstate="collapsed" desc="Tìm các luật kết hợp, có thể có tối đa O(m2^(m-1)) luật (tham khảo Vipin Kumar) ">
        ...
        while (it.hasNext())
        {
            ...
            //int AsssociateRuleRHSCount = Frequency.get(AssociateRuleRHS);
            ...
            {
                //logger.log(Level.OFF, "'{'{0}'}'=>'{'{1}'}' [ supp = {2} | conf = {3} | {4} ]", new Object[]{AssociateRuleLHS, AssociateRuleRHS, (float)AsssociateRuleLHSCount/(float)totalSamples, (float)FreqItemSetStringCount/(float)AsssociateRuleLHSCount,((float)AsssociateRuleLHSCount/(float)totalSamples)/((float)100/(float)totalSamples)}); //For debug only
        ...
        if (args.length==8)
        {
            try
            {
                ...
                if (!"all".equals(args[4]))
                    dbLimit = " limit "+args[4];
                outFileName = args[5];
                minSupport=Float.parseFloat(args[6]);
                minConfidence = Float.parseFloat(args[7]);
                return true;
